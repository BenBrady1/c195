...
public class AppointmentForm extends Form<Appointment> {
    ...
    protected void applyOtherFieldsToRecord() {
        final OffsetDateTime start = parseDateTime(startDatePicker, startHourPicker, startMinutePicker, startMeridiemPicker);
        final OffsetDateTime end = parseDateTime(endDatePicker, endHourPicker, endMinutePicker, endMeridiemPicker);
    ...
    protected void setFields() {
        ...
        setDateFields(record.getStart(), startDatePicker, startHourPicker, startMinutePicker, startMeridiemPicker);
        setDateFields(record.getEnd(), endDatePicker, endHourPicker, endMinutePicker, endMeridiemPicker);
    }
    ...
    private OffsetDateTime parseDateTime(DatePicker datePicker,
                                           ComboBox<String> hourPicker,
                                           ComboBox<String> minutePicker,
                                           ChoiceBox<String> meridiemPicker) {
        final int hour = hourPicker.getSelectionModel().getSelectedIndex() + (meridiemPicker.getSelectionModel().getSelectedItem() == "am" ? 0 : 12);
        final int minute = minutePicker.getSelectionModel().getSelectedIndex();
        return datePicker.getValue().atTime(hour, minute).atZone(ZoneOffset.systemDefault()).toOffsetDateTime();
    }
}
